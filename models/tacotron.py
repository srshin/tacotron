...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, linear_targets=None):
    ...
    with tf.variable_scope('inference') as scope:
      ...
      log(embedded_inputs.shape, '  embedding:               %d' %  embedded_inputs.shape[-1])
      log(prenet_outputs.shape,  '  prenet out:              %d' %  prenet_outputs.shape[-1])
      log(encoder_outputs.shape, '  encoder out:             %d' %  encoder_outputs.shape[-1])
      log('  attention out:           %d' % attention_cell.output_size)
      log('  concat attn & out:       %d' % concat_cell.output_size)
      log('  decoder cell out:        %d' % decoder_cell.output_size)
      log(decoder_outputs.shape,'  decoder out (%d frames): %d' % (hp.outputs_per_step,  decoder_outputs.shape[-1]))
      log(mel_outputs.shape,    '  decoder out (1 frame):   %d' %  mel_outputs.shape[-1])
      log(post_outputs.shape,   '  postnet out:             %d' %  post_outputs.shape[-1])
      log(linear_outputs.shape, '  linear out:              %d' %  linear_outputs.shape[-1])
